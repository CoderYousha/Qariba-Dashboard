...
import AddModel from "../../popup/model/AddModel";
import UpdateModel from "../../popup/model/UpdateModel";
...
function Models() {
    ...
    const getModels = async () => {
        let result = await Fetch(`${host}/api/models?search=${search}`, 'GET', null);
    ...
    const deleteModel = async () => {
        let result = await Fetch(`${host}/api/models/${model.id}`, 'DELETE', null);

        if (result.status === 200) {
            setModels((prevModels) => prevModels.filter((prevModel) => prevModel.id !== model.id));
                ...
                wait ?
                    ...
                    <Box sx={{ backgroundColor: theme.palette.background.default }}>
                                ...
                                getWait ?
                                    ...
                                    <Box sx={{ backgroundColor: theme.palette.background.paper }} className="bg-white rounded-xl px-2">
                                        ...
                                        <Box sx={{ backgroundColor: theme.palette.background.default }} className="flex justify-between items-center px-2">
                                            <Typography variant="h5" className="py-2 px-3 max-sm:!text-lg"><FormattedMessage id='models' /></Typography>
                                                ...
                                                <Box className="min-h-12 py-2 px-2 flex justify-between items-center max-sm:flex-col">
                                                    <Box className="w-full flex items-center">
                                                        <Box className="w-2/4 relative mr-3 max-sm:w-full">
                                                            <input style={{ backgroundColor: theme.palette.background.default }} onChange={(e) => setSearch(e.target.value)} className="w-11/12 h-12 rounded-md border indent-14 outline-none max-sm:w-full" placeholder={intl.formatMessage({ id: "search_model" })} />
                                                    ...
                                                    <Box className="flex w-2/4 items-center justify-end max-sm:mt-2 max-sm:w-full max-sm:justify-between">
                                                        <Typography variant="body1" className="!text-gray-500"><FormattedMessage id='total_models' />: {models.length}</Typography>
                                                    </Box>
                                                </Box>
                                                        ...
                                                        {models.map((model, index) => (
                                                            <StyledTableRow key={index} className="hover:bg-gray-200 duration-100 cursor-pointer">
                                                                <StyledTableCell align={language === 'en' ? "left" : "right"} component="th" scope="row">
                                                                    {
                                                                            <Avatar className="w-10 h-10" src={`${host}/${model.image}`} />
                                                                    }
                                                                </StyledTableCell>
                                                                <StyledTableCell align={language === 'en' ? "left" : "right"} className="">{model.full_name}</StyledTableCell>
                                                                <StyledTableCell align="right">
                                                                    <Box className="!flex justify-center items-center">
                                                                        <Button variant="contained" className="!ml-5 !bg-red-300 !font-bold !text-red-800 hover:!bg-red-500 hover:!text-white duration-300 !mr-2" onClick={(e) => { setModel(model); setPopup('delete', 'flex') }}><FormattedMessage id='delete' /></Button>
                                                                        <Button variant="contained" className="!bg-green-300 !font-bold !text-green-800 hover:!bg-green-500 hover:!text-white duration-300" onClick={(e) => { setModel(model); setPopup('update', 'flex') }}><FormattedMessage id='update' /></Button>
                                                                    </Box>
                                                                </StyledTableCell>
                                                            </StyledTableRow>
                                                        ))}
                        ...
                        <Box id="add" sx={{ right: language === 'en' && '0' }} className="w-4/5 h-screen fixed top-0 bg-gray-200 bg-opacity-5 hidden justify-center items-center">
                            <AddModel setModels={setModels} onClickCancel={() => setPopup('add', 'none')} setSnackBar={setSnackBar} />
                        </Box>
                        ...
                        <Box id="update" sx={{ right: language === 'en' && '0' }} className="w-4/5 h-screen fixed top-0 bg-gray-200 bg-opacity-5 hidden justify-center items-center">
                            <UpdateModel model={model} onClickCancel={() => setPopup('update', 'none')} getModels={getModels} setSnackBar={setSnackBar} />
                        </Box>
                        ...
                        <Box id="delete" sx={{ right: language === 'en' && '0' }} className="w-4/5 h-screen fixed top-0 bg-gray-200 bg-opacity-5 hidden justify-center items-center">
                            <DeleteDialog onClickConfirm={deleteModel} onClickCancel={() => setPopup('delete', 'none')} title={<FormattedMessage id="delete_model_title" />} subtitle={<FormattedMessage id="delete_model_description" />} />
                        </Box>
