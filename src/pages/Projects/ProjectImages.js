...
function ProjectImages() {
    ...
    const deleteImage = async (id) => {
        let result = await Fetch(`${host}/api/project-images/${id}`, 'DELETE');

        if (result.status === 200) {
            setProject((prevProject) => {
                return {
                    ...prevProject,
                    images: prevProject.images.filter((image) => image.id !== id)
                }
            });
    ...
    return (
        <>
                                ...
                                getWait ?
                                    ...
                                    <Box sx={{ backgroundColor: theme.palette.background.default }} className="rounded-xl px-2">
                                        ...
                                        <Box sx={{ backgroundColor: theme.palette.background.default }} className="flex justify-between items-center px-2">
                                            <Typography variant="h5" className="py-2 px-3 max-sm:!text-lg"><FormattedMessage id='images' /></Typography>
                                        ...
                                        <Box className='grid grid-cols-3 mt-10 max-sm:grid-cols-1'>
                                            {
                                                project.images?.map((image, index) =>
                                                    <MediaCard
                                                        ...
                                                        onClickBtn1={() => deleteImage(image.id)}
                                                    />
                                                )
                                            }
                                        </Box>
            ...
            <Box id="add" sx={{ right: language === 'en' && '0' }} className="w-4/5 h-screen fixed top-0 bg-gray-200 bg-opacity-5 hidden justify-center items-center">
                <AddProjectImage setProject={setProject} onClickCancel={() => setPopup('add', 'none')} setSnackBar={setSnackBar} />
            </Box>
